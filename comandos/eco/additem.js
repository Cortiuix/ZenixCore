...
module.exports = {
  ...
  async execute (client, message, args){
     ...
     const f = msg => msg.author.id == message.author.id; message.channel.send('What should be item name?');
      let Name = await message.channel.awaitMessages( {filter:f,
          max: 1
      });
  
      message.channel.send('What should be its price?');
      let Price = await message.channel.awaitMessages({filter:f,
          max: 1
      });
       message.channel.send('What should be its Description?');
      let description = await message.channel.awaitMessages({ filter:f,
          max: 1
      });
