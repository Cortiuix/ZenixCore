...
module.exports = {
    ...
    .setName('pause')
    .setDescription('Pausa la musica del servidor'),
    ...
    async run(client, int){
        ...
        if(!queue.pause){
          int.reply({ content: 'La Musica Ya estaba pausada', ephemeral: true })
          return;
        }
    
        try{
          client.distube.pause(int.member.voice.channel)
          int.reply({ content: 'La Musica fue pausada correctamente' })
          return;
        } catch (e) {
